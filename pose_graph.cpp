...
PoseGraph::PoseGraph()
{
	...
	model = torch::jit::load("/home/ajusd/catkin_ws/src/VINS-Fusion-20.04/loop_fusion/src/feat_extract_script.pt");
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    // std::cout << "Image Size: " << cur_kf->image.size() << std::endl;
    // std::cout << "Image Channels: " << cur_kf->image.channels() << std::endl;
    // std::cout<<"Added\n";
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    ...
    {
        TicToc tmp_t;
        // std::cout<<cur_kf->image<<'\n';
        std::cout<<cur_kf->index<<'\n';
        // loop_index = detectLoop(cur_kf, cur_kf->index);
        loop_index = detectLoopResnet(cur_kf, cur_kf->index);
        std::cout<<"Finish Detecting...\n";
    }
    else
    {
	...
	if (loop_index != -1)
	{
        ...
        std::cout<<"Get KeyFrame\n";

        if (true) //(cur_kf->findConnection(old_kf))
        {
            std::cout<<"Find Connection\n";
...
// Function to apply transformations similar to PyTorch's data_transforms
torch::Tensor PoseGraph::processImage(const cv::Mat& img) {
    // std::cout<<"Converting...\n";
    ...
    // Pad (0, 400, 0, 400) - adding 400 pixels of padding on top and bottom
    // std::cout<<"Before padded: "<<img_rgb.size()<<'\n';
    int top = 400, bottom = 400, left = 0, right = 0;
    cv::Mat img_padded;
    cv::copyMakeBorder(img_rgb, img_padded, top, bottom, left, right, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));

    // std::cout<<"Padded Image: "<<img_padded.size()<<'\n';

    // Resize to 256
    cv::Mat img_resized;
    cv::resize(img_padded, img_resized, cv::Size(256, 256));

    // Center crop to 224x224
    int cropSize = 224;
    int startX = (img_resized.cols - cropSize) / 2;
    int startY = (img_resized.rows - cropSize) / 2;
    cv::Mat img_cropped = img_resized(cv::Rect(startX, startY, cropSize, cropSize));

    // Convert to float and normalize (0-1 range)
    img_cropped.convertTo(img_cropped, CV_32F, 1.0 / 255);

    // Normalize using mean and std (same as PyTorch)
    cv::Mat img_normalized;
    cv::subtract(img_cropped, cv::Scalar(0.485, 0.456, 0.406), img_normalized);
    cv::divide(img_normalized, cv::Scalar(0.229, 0.224, 0.225), img_normalized);

    // Convert OpenCV Mat to Torch Tensor
    torch::Tensor img_tensor = torch::from_blob(img_normalized.data, {1, 224, 224, 3}, torch::kFloat32);

    // Permute (H, W, C) → (C, H, W) (PyTorch format)
    img_tensor = img_tensor.permute({0, 3, 1, 2});

    return img_tensor.clone();  // Clone to ensure memory safety
}

int PoseGraph::detectLoopResnet(KeyFrame* keyframe, int frame_index) {
    // std::cout<<"start\n";
    // std::cout << "Image Size at Loop: " << keyframe->image.size() << std::endl;
    // std::cout<<"channels: "<<keyframe->image.channels()<<'\n';
    torch::NoGradGuard no_grad_guard;
    // Process the image
    torch::Tensor input_tensor1 = processImage(keyframe->image).to(torch::kCUDA);
    // std::cout<<"end\n";

    // Print the tensor shape
    // std::cout << "Processed Tensor Shape: " << input_tensor1.sizes() << std::endl;

    // // Example forward.
    std::vector<torch::jit::IValue> input1;
    // input.push_back(torch::zeros({1, 3, 224, 224}));

    input1.push_back(input_tensor1);

    auto output1 = model.forward(input1).toTuple();

    torch::Tensor layer4_output1 = output1->elements()[3].toTensor().to(torch::kCUDA);

    // Remove intermediate tensors from GPU memory
    // input_tensor1 = torch::Tensor();
    // output1.reset();  // Release JIT tuple object
    // layer4_output1 = torch::Tensor();

    // Force garbage collection on GPU
    // c10::cuda::CUDACachingAllocator::emptyCache();
    // torch::cuda::synchronize();

     // Print shape of layer4 feature map
    // std::cout << "Layer 4 Feature Map Shape: " << layer4_output1.sizes() << std::endl;

    // // Simulating feature tensors from layer4
    // torch::Tensor feature1 = torch::randn({1, 2048, 7, 7}); // Example feature map 1
    // torch::Tensor feature2 = torch::randn({1, 2048, 7, 7}); // Example feature map 2

    torch::Tensor keyframe_feature;

    // Flatten feature maps to 1D vectors
    keyframe_feature = layer4_output1.flatten(1).to(torch::kCPU);  // Shape: [1, 2048 * 7 * 7]

    // Search in previous frames
    int best_match = -1;
    double best_score = 0.0;

    torch::Tensor cosine_sim;
    torch::Tensor similarity_score;

    // std::cout << "CUDA Available: " << torch::cuda::is_available() << std::endl;

    
    if (feature_database.size() > 1) {
        for (auto& [index, stored_feature] : feature_database)
        {
            // double score = computeSimilarity(keyframe_feature, stored_feature);
            // std::cout<<"Index: "<<index<<'\n';
            // std::cout<<"Stored value: "<<stored_feature<<'\n';
            // Compute cosine similarity
            cosine_sim = torch::cosine_similarity(keyframe_feature, stored_feature, 1); // dim=1

            // Print result
            // std::cout << "Cosine Similarity Score: " << cosine_sim << std::endl;

            // Normalize to range [0, 1]
            cosine_sim = (cosine_sim + 1) / 2;

            // Aggregate similarity score (mean)
            similarity_score = cosine_sim.mean();

            // Print result
            // std::cout << "Cosine Similarity Score: " << similarity_score.item<float>() << std::endl;
            if (similarity_score.item<float>() > 0.89 && similarity_score.item<float>() > best_score)
            {
                if (abs(frame_index - index) > 50) {
                    std::cout << "Cosine Similarity Score: " << similarity_score.item<float>() << std::endl;
                    best_match = index;
                    best_score = similarity_score.item<double>();
                    std::cout << "Loop at: (" << frame_index << ", " << best_match << ")\n";
                }
            }
        }
    }
    ...
    // Add current frame to feature database
    feature_database[frame_index] = keyframe_feature;

    // keyframe_feature = keyframe_feature.to(torch::kCPU);
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    std::cout<<"channels: "<<keyframe->image.channels()<<'\n';
    std::cout << "Image Size: " << keyframe->image.cols << " x " << keyframe->image.rows << std::endl;
    TicToc tmp_t;
