...
void process()
{
    while (true)
    {
        ...
        m_buf.unlock();
        std::cout << "Number of elements in image_buf: " << image_buf.size() << std::endl;
        ...
        if (pose_msg != NULL)
        {
            ...
            if (image_msg->encoding == "8UC1")
            {
                ...
                img.encoding = "bgr8"; //mono8
                ptr = cv_bridge::toCvCopy(img, sensor_msgs::image_encodings::BGR8); //MONO8
            }
            else
                ptr = cv_bridge::toCvCopy(image_msg, sensor_msgs::image_encodings::BGR8); //MONO8
            
            cv::Mat image = ptr->image;
            // std::cout<<"Image Channel: "<<image.channels()<<'\n';
            // build keyframe
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                // std::cout<<"Initial Image: "<<keyframe->image.size()<<'\n';  
                m_process.lock();
                start_flag = 1;
