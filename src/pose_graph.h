...
private:
	...
	void optimize6DoF();
	void updatePath();
	torch::Tensor processImage(const cv::Mat& img);
	int detectLoopResnet(KeyFrame* keyframe, int frame_index);
	...
	// Normalize constants (same as torchvision.transforms)
	const std::vector<float> mean = {0.485, 0.456, 0.406};
	const std::vector<float> std_dev = {0.229, 0.224, 0.225};
	std::unordered_map<int, torch::Tensor> feature_database;
