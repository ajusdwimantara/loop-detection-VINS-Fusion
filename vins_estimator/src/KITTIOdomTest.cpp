...
int main(int argc, char** argv)
{
	...
	{	
		if(ros::ok())
		{
			...
			imLeft = cv::imread(leftImagePath, cv::IMREAD_COLOR);
			sensor_msgs::ImagePtr imLeftMsg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", imLeft).toImageMsg();
			imLeftMsg->header.stamp = ros::Time(imageTimeList[i]);
			pubLeftImage.publish(imLeftMsg);

			imRight = cv::imread(rightImagePath, cv::IMREAD_COLOR);
			sensor_msgs::ImagePtr imRightMsg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", imRight).toImageMsg();
			imRightMsg->header.stamp = ros::Time(imageTimeList[i]);
			pubRightImage.publish(imRightMsg);

			std::cout<<"Image channels: "<<imLeft.channels()<<'\n';

			estimator.inputImage(imageTimeList[i], imLeft, imRight);

			std::cout<<"Finish Input Image\n";
